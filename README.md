# Orto-N-L-M

> AI-система для обучения сотрудников и формирования базы знаний компании.

## О проекте

Orto-N-L-M собирает знания сотрудников через интервью и превращает их в структурированную базу знаний, на основе которой можно автоматически создавать обучающие материалы.

Проект разрабатываю самостоятельно: от архитектуры и проектирования workflow до интеграций, базы данных и развёртывания на VPS.

## Демонстрация

Типовой сценарий работы системы:

1. Сотрудник запускает интервью через Telegram.
2. AI проводит интервью и собирает информацию о рабочем процессе.
3. Полученная информация структурируется в Process JSON.
4. Данные сохраняются в PostgreSQL.
5. На основе Process JSON автоматически формируется обучающий материал.
6. Готовый результат передаётся пользователю через Telegram.

### Скриншоты

Скриншоты демонстрируют полный путь от взаимодействия с пользователем до готового результата:

1. **Интервью в Telegram** — AI собирает информацию о должности и рабочем процессе сотрудника.
2. **Workflow в n8n** — основная архитектура обработки данных и генерации результата.
3. **Process JSON** — структурированное представление знаний, которое используется как канонический источник данных.
4. **Готовая инструкция** — конечный обучающий материал, сформированный системой.

> Изображения будут добавлены в `docs/screenshots/`.

## Что уже сделано

- архитектура проекта;
- Telegram Input Gateway;
- маршрутизация сообщений;
- управление сессией интервью;
- Interview Agent;
- Process JSON;
- PostgreSQL;
- сохранение состояния интервью;
- генерация инструкции;
- формирование Markdown-файла;
- отправка готового результата пользователю в Telegram;
- развёртывание и отладка проекта на VPS.

## Архитектура

```text
Telegram
    ↓
Input Gateway
    ↓
Interview Session Router
    ↓
Interview Agent
    ↓
Process JSON
    ↓
PostgreSQL
    ↓
Knowledge Generator
    ↓
Instruction / Learning Material
    ↓
File
    ↓
Telegram
```

## Основной принцип

**Process JSON — единственный источник истины.**

Знания хранятся отдельно от их представления. AI используется для обработки информации и генерации материалов, а структурированные данные остаются каноническим источником, из которого можно создавать разные форматы обучающих материалов.

## Что сделал я

- спроектировал архитектуру системы;
- разработал workflow в n8n;
- реализовал логику интервью и управления сессией;
- спроектировал структуру Process JSON;
- настроил интеграцию с Telegram;
- настроил работу с OpenAI API;
- реализовал сохранение данных в PostgreSQL;
- реализовал генерацию обучающих материалов;
- настроил формирование и передачу файлов;
- развернул проект на VPS;
- занимаюсь тестированием, отладкой и дальнейшим развитием системы.

## Стек

- Ubuntu Server 24.04 LTS
- Docker 29.6.1
- Docker Compose 2.39.x
- PostgreSQL 17.6
- pgvector 0.8.2
- n8n 2.29.10 Self Hosted
- OpenAI API
- Telegram Bot API
- JavaScript

## Статус

🚧 Проект активно развивается.

Основной MVP-цикл уже собран: от интервью с сотрудником до сохранения структурированных знаний, генерации обучающего материала и передачи готового результата пользователю.

Следующие этапы — расширение генераторов, базы знаний, тестирования и AI Assistant.

## Автор

**Алексей Хорьяков**

AI Automation / AI Integration

[GitHub](https://github.com/AlekseyHorjakov)
